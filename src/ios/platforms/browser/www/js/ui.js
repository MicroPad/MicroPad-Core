...
appUi.onPageInit('index', () => {updateNotepadList();});
appUi.onPageReinit('index', updateNotepadList);

function updateNotepadList() {
	notepadStorage.iterate().then((value, key, i) => {
		$$('#notepadList').html('<li class="item-content"><div class="item-inner"><div class="item-title">{0}</div></div></li>'.format(key));
	});
}
